bank_admin = new Vue({
    ...
    data: {
        ...
        BankCustom: [{ "type": "Chuyển khoản ngân hàng" }, { "type": "Ví điện tử" }],
        loaiBank: "",
        bankName: "",
        bankAccount: "",
        nameAccount: "",
    ...
    methods: {
        ...
        onFileChange2(event) {
            ...
            this.previewImage2 = URL.createObjectURL(this.imageFile12);
        ...
        async addBank() {
            try {
                ...
                formData.append('LoaiBank', this.loaiBank);
                formData.append('BankName', this.bankName);
                formData.append('BankAcc', this.bankAccount);
                formData.append('BankUser', this.nameAccount);
                formData.append('ImgPr', this.$refs.PrPath.files[0]);
                if (this.$refs.PrPath2.files[0] != null ) {
                    formData.append('ImgPrLogo', this.$refs.PrPath2.files[0]);
                }
                else {
                    formData.append('ImgBank', this.selectedBank);
                }
              
               

                await axios.post('/Bank/AddBankData', formData,
                    {
                        headers: {
        ...
        getItemsById(id) {
            axios.get(`/Bank/getIdBank/${id}`)
                .then((response) => {
                    //this.loaiBank = this.BankCustom.find(bank => bank.type === response.data.loaiBank) || {};
                    //console.log(this.loaiBank);
                    this.loaiBank = response.data.loaiBank;
                    this.bankName = response.data.bankName;
                    this.bankAccount = response.data.bankAcc;
                    this.id = response.data.id;
                    this.nameAccount = response.data.bankUser;
        ...
        async editBank() {
            try {
                ...
                if (this.$refs.PrPath1.files[0] != null) {
                    formData.append('ImgPr', this.$refs.PrPath1.files[0]);

                }
                await axios.post(`/Bank/editBank/${this.id}`, formData,
                    {
                        headers: {
                            'Content-Type': 'multipart/form-data'
                        }
                    ...
                    if (this.id != null) {
                        Swal.fire({
                            ...
                            if (result.isConfirmed) {
                                const formData = new FormData();
                                formData.append('id', this.id);
                                axios.post('/Bank/deleteBankAdmin', formData, {
                                    headers: {
