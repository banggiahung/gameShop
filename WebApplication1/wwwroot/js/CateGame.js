Admin_category = new Vue({
    ...
    mounted() {
        ...
        axios.get("/Products/GetAllSub")
            .then((response) => {
                this.dataItemsSub = response.data;
                return Promise.resolve();
            })
            .then(() => {
                $("#cateSub_table").DataTable({
                    'columnDefs': [{
                        'targets': [-1],
                        'orderable': false,
                    }],
    ...
    methods: {
            ...
            try {
                if (this.CategoryID == 0) {
                    Swal.fire({
                        icon: 'error',
                        title: 'Lỗi',
                        text: 'Không được để trống mục thuộc danh mục',
                        confirmButtonText: 'OK'
                    });
                    return;
                }
                const formData = new FormData();
                formData.append('NameSub', this.SubName);
                formData.append('IdCate', this.CategoryID);
        ...
        getItemsById(id) {
            axios.get(`/Products/getIdCategory?id=${id}`)
                .then((response) => {
                    ...
                    this.imageCategory = response.data.mainImgCate;
        ...
        getItemsSubById(id) {
            axios.get(`/Products/getIdSubMenu?id=${id}`)
                .then((response) => {
                    ...
                    this.SubName = response.data.nameSub;
                    this.IdSub = response.data.id;
                    this.CategoryID = response.data.idCate;
            ...
            axios.get(`/Products/getIdCategory/${id}`)
                .then((response) => {
                    ...
                    if (this.categoryID != null) {
                        Swal.fire({
                            title: 'Xóa danh mục',
                            text: 'Bạn có chắc chắn muốn xóa, nếu có menu con, các menu con sẽ bị xóa',
                            icon: 'warning',
        ...
        getItemsSubByIdDelete(id) {
            axios.get(`/Products/getIdSubMenu/${id}`)
                .then((response) => {
                    this.IdSub = response.data.id;
                    if (this.IdSub != null) {
                        Swal.fire({
                            title: 'Xóa danh mục phụ',
